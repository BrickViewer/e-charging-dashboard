-- Atomaire klant-aanmaak vanuit een getekende offerte. Alles in één plpgsql-functie = één transactie:
-- faalt een stap, dan rolt ALLES terug (geen half-aangemaakte klant meer). De edge fn quote-create-client
-- blijft de auth-gate en roept deze functie aan als service_role.
...
begin
  ...
  -- Gereviewde gegevens (reviewed → offerte → lead). Tarieven NIET hier (die staan per locatie).
