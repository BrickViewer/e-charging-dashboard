-- Kwaliteitsscore zichtbaar op de blog zelf. Tot nu toe schreef content_ingest_draft de
-- (poort-bepalende!) quality_score alleen naar content_topics; de admin-UI kon hem daardoor
-- nergens tonen. Kolom + backfill + de poort-RPC schrijft hem voortaan óók op blog_posts.
-- Signature van de RPC blijft byte-voor-byte gelijk (CREATE OR REPLACE, ACL blijft staan).

alter table public.blog_posts add column if not exists quality_score int;

-- Backfill vanuit het gekoppelde onderwerp (bron van waarheid tot nu toe).
update public.blog_posts bp
   set quality_score = ct.quality_score
  from public.content_topics ct
 where bp.source_topic_id = ct.id
   and bp.quality_score is null
   and ct.quality_score is not null;

-- Kopie van 20260623160000_content_draft_and_distribution.sql met exact twee wijzigingen:
-- quality_score in de INSERT-kolomlijst + p_quality_score in VALUES.
