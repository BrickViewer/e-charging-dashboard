-- create_client_from_quote: koppel de al-bij-tekenen aangemaakte clientloze
-- installatie-order aan de nieuwe klant (die UPDATE was ooit weggevallen). Nodig
-- nu 'klantaccount aanmaken' direct na tekenen komt (vóór facturering): de order
-- bestaat dan al clientloos en moet aan de verse klant gekoppeld worden, anders
-- heeft de klant geen installatie-order en loopt de installateur-track niet.
-- Idempotente CREATE OR REPLACE; enige inhoudelijke wijziging = het
-- installation_orders-blok (UPDATE-koppelen + INSERT-als-geen).
...
begin
  ...
  if p_target_client_id is not null then
    ...
    update public.clients set activation_fee_total = coalesce(activation_fee_total,0) + v_activation where id = v_client;
    ...
    if v_client is not null then
      update public.clients set
        ...
        managed=f_managed, needs_installation=f_needs_install, status='actief', activation_fee_total = coalesce(activation_fee_total,0) + v_activation
      where id = v_client;
    end if;
  end if;
  ...
  -- Installatie-order: koppel de bij tekenen aangemaakte clientloze order aan de
  -- nieuwe klant; bestaat er nog geen (edge case), maak er dan een.
  if f_needs_install then
    update public.installation_orders set client_id = v_client where quote_id = q.id and client_id is null;
