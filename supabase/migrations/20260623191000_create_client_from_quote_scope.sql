-- create_client_from_quote: installatie-as toevoegen. Zet clients.needs_installation (uit
-- quote.with_installation of review) en maak de installation_order ALLEEN aan als er installatie is
-- (alleen-beheer → geen install-order → geen installateur-handoff). managed/object-link ongewijzigd.
...
begin
  ...
  if v_client is null then
    insert into public.clients (organization_id, company_id, person_id, company_name, kvk, btw_number, contact_name,
      ...
      contract_duration_months, notice_period_months, managed, needs_installation, status, notes)
    values (v_org, v_company, v_person, f_company_name, f_kvk, f_btw, f_contact_name, f_contact_email, f_contact_phone,
      f_street, f_postal, f_city, coalesce(f_duration, 12), coalesce(f_notice, 3), f_managed, f_needs_install, 'actief', 'Aangemaakt via offerte ' || coalesce(q.quote_number, ''))
