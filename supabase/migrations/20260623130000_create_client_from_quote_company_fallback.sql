-- create_client_from_quote: bedrijfsgegevens (kvk/btw/naam/adres) ook uit het GEKOPPELDE BEDRIJF
-- halen als fallback (companies = bron van waarheid). Voorheen kwam kvk alleen uit p_reviewed/lead en
-- btw alleen uit p_reviewed — waardoor een via het bedrijf ingevulde KvK/BTW niet op de klant belandde.
-- Volgorde: expliciete review (p_reviewed) → bedrijf → lead. Rest van de functie ongewijzigd.
create or replace function public.create_client_from_quote(
  p_quote_id uuid,
  p_reviewed jsonb default '{}'::jsonb,
...
begin
  ...
  -- Gereviewde gegevens (reviewed → bedrijf → offerte → lead). Tarieven NIET hier (die staan per locatie).
  f_company_name := coalesce(nullif(btrim(p_reviewed->>'company_name'), ''), v_co.name, q.prospect_company, l.company_name, 'Onbekend bedrijf');
  ...
  -- Contactenlaag terugschrijven (best-effort; unique-violations negeren).
  ...
  -- Klant bepalen: bestaand (expliciet) → hergebruik-per-bedrijf → nieuw.
  if p_target_client_id is not null then
    ...
    if v_client is not null then
      update public.clients set
        ...
        managed=f_managed, status='actief'
      where id = v_client;
    end if;
  end if;
  ...
  -- Configuratie-snapshot (nieuwe versie).
  ...
  -- Installatie-order (één per offerte).
  ...
  -- Dossier koppelen (alleen bij beheer).
...
revoke all on function public.create_client_from_quote(uuid, jsonb, uuid) from public, anon, authenticated;
grant execute on function public.create_client_from_quote(uuid, jsonb, uuid) to service_role;
