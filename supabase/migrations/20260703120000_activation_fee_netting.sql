-- Netting of one-time activatiekosten against the client's first monthly settlement(s).
-- client_payout stays GROSS ("vergoeding"); settlements.activation_cost holds the deducted amount.

-- 1. Columns (idempotent)
alter table public.clients add column if not exists activation_fee_total numeric not null default 0;
alter table public.settlements add column if not exists activation_cost numeric not null default 0;
comment on column public.clients.activation_fee_total is 'Eenmalige activatiekosten (excl. BTW) te verrekenen met de eerste vergoedingsfactuur(en). Uit de getekende offerte.';
comment on column public.settlements.activation_cost is 'Deel van de activatiekosten dat in deze afrekening verrekend is (excl. BTW). Netto uitbetaald = client_payout - activation_cost. client_payout blijft de bruto vergoeding.';

-- 2. Portal dashboard KPIs: expose activation_cost (identical to prior definition + one column)
-- Adding an output column changes the return type, so a DROP is required before recreating.
DROP FUNCTION IF EXISTS public.get_portal_dashboard_kpis();
CREATE OR REPLACE FUNCTION public.get_portal_dashboard_kpis()
 RETURNS TABLE(year integer, month integer, period_start date, period_end date, status text, is_final boolean, total_kwh numeric, total_customer_cashflow numeric, estimated_client_yield numeric, co2_kg_avoided numeric, ere_estimate numeric, activation_cost numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Restore the exact grants the function had before the DROP (PUBLIC stays revoked).
...
-- 3. create_client_from_quote: capture activation on the NEW-client insert only
...
begin
  ...
  v_activation := case when f_needs_install then coalesce(nullif(q.offer_details->>'activatiekostenPerSocket','')::numeric, 0) * coalesce(q.num_charge_points, 0) else coalesce(q.total_installation_cost, 0) end;
  ...
  if v_client is null then
    insert into public.clients (organization_id, company_id, person_id, company_name, kvk, btw_number, contact_name,
      ...
      contract_duration_months, notice_period_months, managed, needs_installation, vat_status, status, notes, activation_fee_total)
    values (v_org, v_company, v_person, f_company_name, f_kvk, f_btw, f_contact_name, f_contact_email, f_contact_phone,
      f_street, f_postal, f_city, coalesce(f_duration, 12), coalesce(f_notice, 3), f_managed, f_needs_install, f_vat_status, 'actief', 'Aangemaakt via offerte ' || coalesce(q.quote_number, ''), v_activation)
  ...
  if q.project_location_id is not null then
    update public.project_locations set client_id = v_client where id = q.project_location_id and client_id is null;
  end if;
