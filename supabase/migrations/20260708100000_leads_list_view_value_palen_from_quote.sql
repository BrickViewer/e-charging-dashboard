-- Waarde + palen in de lijst uit de primaire offerte (zoals het bord), i.p.v. de rauwe
-- estimated_value/estimated_charge_points. Eén lateral join op de nieuwste relevante offerte
-- levert palen, kosten en scope-flags. paal_count/quote_value/mgmt_in_scope zijn nieuw (achteraan).
...
select
  ...
  coalesce(pq.num_charge_points, l.estimated_charge_points) as paal_count,
  (coalesce(pq.total_hardware_cost, 0) + coalesce(pq.total_installation_cost, 0)) as quote_value,
