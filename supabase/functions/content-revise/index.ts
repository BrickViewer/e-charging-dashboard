...
// en: haalt het de HOGE lat → publiceren; anders < MAX → zichzelf re-invoken met de nieuwe kritiek; anders (geplateaud)
// → publiceren als het boven de vloer zit, anders concept laten. Zo verschijnt er vanzelf een sterke blog, hands-off.
// verify_jwt=false; intern (cron/keten) of admin/marketing. Body: { blog_post_id, iteration?, issues?, missing_experience? }.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // EÉN herschrijf-call (GEEN inline-repair: twee Claude-calls zouden de 150s-edgelimiet raken → 504). Ruime
    // maxTokens tegen JSON-afkap bij een uitgebreide herschrijf. Faalt de JSON, dan proberen we het in een VERSE
    // schakel opnieuw (eigen tijdsbudget) i.p.v. duur inline te repareren.
    ...
    try {
      ...
      if (finalize) {
        return json({ status: "ok", action: "finalize_jsonfail", blog_post_id: blogPostId, reason: e instanceof Error ? e.message.slice(0, 140) : "json", ms: Date.now() - runStart });
      }
      if (iteration < MAX) {
    ...
    // Verbeterde versie opslaan (slug blijft; categorie wordt (her)berekend).
    ...
    // content-update triggert de site-rebuild. Zo brengen we oude blogs definitief op de huisstijl zonder ze te
    // depubliceren of hun URL te wijzigen.
    if (finalize) {
      // Site-rebuild forceren via een expliciete published_at-touch is niet nodig; de update-trigger vuurt al.
      return json({ status: "ok", action: "finalized", blog_post_id: blogPostId, slug: post.slug, category_slugs: catSlugs, ms: Date.now() - runStart });
    }
    ...
    if (passesHigh) return await publish("high_bar");
    ...
    if (iteration < MAX) {
      ...
      await sb.rpc("invoke_edge_function", {
        ...
        body: { blog_post_id: blogPostId, iteration: iteration + 1, issues: audit.issues, missing_experience: audit.missing_experience },
      });
      return json({ status: "ok", action: "revising", blog_post_id: blogPostId, iteration, next: iteration + 1, scores: { q, seo, aeo }, verdict: audit.verdict, ms: Date.now() - runStart });
    }
