...
export function hasValidIbanCountryLength(value: string) {
  const iban = normalizeIban(value);
  const expectedLength = IBAN_COUNTRY_LENGTHS[iban.slice(0, 2)];
  return Boolean(expectedLength) && iban.length === expectedLength;
}

export function isValidIban(value: string) {
  const iban = normalizeIban(value);
  if (!/^[A-Z]{2}[0-9]{2}[A-Z0-9]+$/.test(iban)) return false;
  if (!hasValidIbanCountryLength(iban)) return false;
