 ...
 * De onderliggende cijfers zitten achter een chevron per regel.
 */
  ...
  return (
    <div className="overflow-hidden rounded-xl border bg-card">
      ...
      {/* Arbeid & voorrijkosten: twee vaste regels in dezelfde vorm als een
          artikelregel. De urenregels waaruit het totaal is opgebouwd zitten
          achter de chevron van Uurloon. */}
      <section data-testid="section-arbeid">
        ...
        <FixedRow
          ...
          qty={<span className="w-full px-1 text-right text-sm tabular-nums">{getal(totals.hoursTotal)}</span>}
          label="Uurloon"
          subline={
            <>
              <span>€</span>
            ...
            <>
              {urenLines.map((line) => (
                <div key={line.uid} className="flex items-center gap-2">
                  <NumField
                    className="h-7 w-12 px-1 text-right text-xs tabular-nums"
                    value={line.qty}
                    disabled={frozen}
                    onCommit={(n) => onPatchLine(line.uid, { qty: n })}
                  />
                  <Input
                    className={cn("h-7 min-w-0 flex-1 px-1 text-xs", GHOST)}
                    value={line.description}
                    placeholder="Omschrijving werkzaamheden…"
                    disabled={frozen}
                    onChange={(e) => onPatchLine(line.uid, { description: e.target.value })}
                  />
                  <span className="w-12 shrink-0 text-right text-xs tabular-nums text-muted-foreground">{uren(lineTotals(line).hours)}</span>
              ...
              {urenLines.length === 0 && <p className="py-1 text-center text-xs text-muted-foreground">Nog geen arbeidsregels</p>}

              {/* Deze uren zitten op materiaalregels (een meterkast draagt er 8)
                  en tellen mee in het montagebedrag — daarom hier zichtbaar. */}
              {fromProductLines > 0 && (
                <div className="flex items-center gap-2 border-t border-border/60 pt-1.5 text-xs text-muted-foreground">
                  <span className="min-w-0 flex-1 truncate">Calculatietijd uit materiaalregels</span>
                  <span className="w-12 shrink-0 text-right tabular-nums">{uren(fromProductLines)}</span>
        ...
        <FixedRow
          ...
          detail={
            <div className="space-y-1.5 text-[11px] text-muted-foreground">
              <p>Retour-afstand tussen kantoor en projectlocatie.</p>
              {kmHint && <p>{kmHint}</p>}
              {!frozen && (
    ...
    <CalcRow
      ...
      main={
        <>
          ...
          {expanded && (
            <div id={detailId} className="mb-1 mt-1 space-y-1 rounded-md bg-muted/50 px-2 py-2">
              {detail}
            </div>
          )}
        </>
        ...
        <>
          ...
          {expanded && (
            <div id={detailId} className="mb-1 mt-1 space-y-1 rounded-md bg-muted/50 px-2 py-1.5">
              ...
              />
              <DetailField
                label={`Montagetijd per ${line.unit}`}
