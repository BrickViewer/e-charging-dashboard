...
export function StoringDetailBody({ faultId, onClose }: { faultId: string; onClose: () => void }) {
  const fault = useFault(faultId);
  const f = fault.data;
  ...
  if (fault.isError) {
    return (
      <div className="space-y-4">
        <Button variant="ghost" onClick={onClose}><ArrowLeft className="w-4 h-4 mr-1.5" /> Terug</Button>
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between gap-3">
        <Button variant="ghost" onClick={onClose}><ArrowLeft className="w-4 h-4 mr-1.5" /> Storingen</Button>
