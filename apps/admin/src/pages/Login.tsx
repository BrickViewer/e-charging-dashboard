...
export default function Login({ mode = "client" }: { mode?: "client" | "admin" }) {
  ...
  const handlePasswordReset = async () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)) {
      setErrMsg("Vul eerst uw e-mailadres in");
      setPhase("error");
      return;
    }
    ...
    try {
      ...
      toast.success("Controleer uw e-mail voor de herstel-link");
    } catch (err) {
      const message = err instanceof Error ? err.message : "Herstel-link versturen mislukt";
      setErrMsg(message);
      setPhase("error");
    } finally {
