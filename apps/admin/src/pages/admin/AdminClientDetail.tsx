...
// Route-wrapper: houdt de externe deeplinks (/admin/klanten/:id) werkend. De volledige inhoud
// zit in ClientDetailBody, die ook door de slide-over (ClientDetailSheet) wordt hergebruikt.
export default function AdminClientDetail() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  return <ClientDetailBody clientId={id!} onClose={() => navigate("/admin/klanten")} />;
}
