...
export const defaultConfiguratorSettings = configuratorSettingsSchema.parse({
...
export const excelDefaultPricingInput = pricingInputSchema.parse({
    customer: {
...
function findTier(tiers, netReturn) {
...
function calculateWith(input, settings, override) {
    ...
    const idleFeeRevenuePerChargePointMonth = tariffs.idleFeeEnabled
        ...
        : 0;
    const netReturnPerChargePointMonth = grossChargingRevenuePerChargePointMonth +
    ...
    const efluxCostPerSocketMonth = settings.efluxSubscriptionPerSocketMonth +
...
export function calculatePricing(rawInput, rawSettings = defaultConfiguratorSettings) {
