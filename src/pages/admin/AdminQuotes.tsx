import { useAllQuotes } from "@/hooks/useAdminData";
...
export default function AdminQuotes() {
  ...
  const filtered = (quotes || []).filter((q: any) => {
    const company = q.prospect_company || q.clients?.company_name || "";
    const matchesSearch =
      company.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
      (q.quote_number || "").toLowerCase().includes(debouncedSearch.toLowerCase());
    const matchesStatus = statusFilter === "alle" || q.status === statusFilter;
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/offertes/nieuw">
          <Button><Plus className="w-4 h-4 mr-2" />Nieuwe offerte</Button>
        </Link>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative max-w-sm flex-1 min-w-[200px]">
          ...
          <Input placeholder="Zoek offerte..." value={search} onChange={e => handleSearch(e.target.value)} className="pl-9" />
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left p-3 font-medium text-muted-foreground">Offertenr</th>
                  ...
                  <th className="text-right p-3 font-medium text-muted-foreground">Totaal/jaar</th>
              ...
              <tbody>
                ...
                {!isLoading && paginated.map((q: any) => {
                  const company = q.prospect_company || q.clients?.company_name || "—";
                  const yearly = getYearlyTotal(q);
                  return (
                    <tr
                      key={q.id}
                      className="border-b border-border last:border-0 hover:bg-accent/50 cursor-pointer"
                      onClick={() => navigate(`/admin/offertes/${q.id}`)}
                    >
                      <td className="p-3 font-medium font-mono">{q.quote_number || "—"}</td>
                      <td className="p-3">{company}</td>
                      <td className="p-3 text-right">{q.num_charge_points ?? "—"}</td>
                      <td className="p-3 text-right">{yearly != null ? formatEuro(yearly) : "—"}</td>
                      <td className="p-3"><StatusBadge status={q.status || "concept"} /></td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between p-3 border-t border-border">
              <span className="text-sm text-muted-foreground">
                {filtered.length} offertes — pagina {page + 1} van {totalPages}
              </span>
