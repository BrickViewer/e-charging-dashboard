...
export default function ClientFinancial() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <Card className="portal-card">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Gem. opbrengst per kwartaal</p>
                <p className="text-lg font-semibold">{fmt(kpis.avgQuarterly)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {filtered.map((s: any) => {
        const subscriptionCosts = Number(s.total_platform_fee || 0) + Number(s.total_transaction_fees || 0) + Number(s.ere_commission || 0);
        return (
          <Card key={s.id}>
            <CardContent className="p-6">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>-{fmt(subscriptionCosts)}</span>
                </div>
                ...
                <div className="flex justify-between text-lg font-bold">
                  <span>Uw uitbetaling ({client?.revenue_share_percentage || 75}%)</span>
                  <span className="text-primary">{fmt(Number(s.client_payout))}</span>
                </div>
              ...
              <p className="text-xs text-muted-foreground mt-4 italic">
                Uw uitbetaling bevat zowel het revenue-share-deel als de ERE-opbrengst. Stroominkoop wordt afzonderlijk vergoed op basis van het contract-tarief.
              </p>
            </CardContent>
          </Card>
        );
